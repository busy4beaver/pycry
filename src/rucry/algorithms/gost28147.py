...
S-боксы, ключ, режим и синхропосылка (IV) задаются при создании класса
или передаются в низкоуровневый crypt().
...
Usage (новый стиль):
    ...
    g = Gost28147(key=key, sbox="cryptopro-a", mode=CRYPT_MODE.CBC, iv=b"\\x00"*8)
    ct = g.encrypt(b"hello!!!")
    pt = g.decrypt(ct)

Usage (старый стиль, сохранён):
...
class Gost28147:
    ...
    Параметры можно задать один раз в __init__ (ключ, sbox, mode, iv, padding),
    после чего пользоваться encrypt() / decrypt().
    Низкоуровневый crypt() сохранён для совместимости и гибкости.
    """
    ...
    def __init__(
        ...
        Parameters
        ----------
        key : 8×uint32 или 32 байта. Можно не указывать и передавать в crypt().
        sbox : имя ("cryptopro-a", "test", …) или 8×16 таблица. Default — cryptopro-a.
        mode : CRYPT_MODE.ECB / CBC / OFB / CTR.
        iv : синхропосылка, ровно 8 байт. Default — нули.
        padding : "zero" | "pkcs7" | "ansi" | "none". Используется только в ECB/CBC при pad=True.
        pad : включать ли паддинг (для encrypt в ECB/CBC).
        """
        self._sbox = resolve_sbox(sbox)
        self._key: Optional[List[int]] = normalize_key(key) if key is not None else None
        self._mode = CRYPT_MODE(mode)
        self._iv = bytearray(iv) if iv is not None else bytearray(8)
        if len(self._iv) != 8:
            raise ValueError("iv (синхропосылка) must be 8 bytes")
        padding = padding.lower().strip()
        if padding not in ("zero", "pkcs7", "ansi", "none"):
            raise ValueError('padding must be "zero", "pkcs7", "ansi" or "none"')
    ...
    @property
    def mode(self) -> CRYPT_MODE:
        return self._mode

    @mode.setter
    def mode(self, value: CRYPT_MODE) -> None:
        self._mode = CRYPT_MODE(value)
    ...
    @iv.setter
    def iv(self, value: Sequence[int]) -> None:
        ba = bytearray(value)
        if len(ba) != 8:
            raise ValueError("iv (синхропосылка) must be 8 bytes")
        self._iv = ba
    ...
    def padding(self, value: str) -> None:
        ...
        if value not in ("zero", "pkcs7", "ansi", "none"):
            raise ValueError('padding must be "zero", "pkcs7", "ansi" or "none"')
        self._padding = value
    ...
    def set_sbox(self, sbox: Union[str, Sequence[Sequence[int]], None]) -> None:
        self._sbox = resolve_sbox(sbox)
    ...
    def encrypt(
        ...
        *,
        key: Optional[Sequence[int]] = None,
        mode: Optional[CRYPT_MODE] = None,
        iv: Optional[Sequence[int]] = None,
        padding: Optional[str] = None,
    ) -> bytes:
        """
        Зашифровать данные. Возвращает bytes.

        Параметры, не переданные явно, берутся из настроек экземпляра.
        """
        ba = to_bytearray(data)
        used_key = key if key is not None else self._key
        if used_key is None:
            raise ValueError("key is not set (pass key=... to __init__ or encrypt)")
        used_mode = CRYPT_MODE(mode) if mode is not None else self._mode
        used_iv = bytearray(iv) if iv is not None else bytearray(self._iv)
        used_padding = (padding or self._padding).lower()

        if used_mode in (CRYPT_MODE.ECB, CRYPT_MODE.CBC) and self._pad and used_padding != "none":
            if used_padding == "pkcs7":
                ba = pkcs7_pad(ba, 8)
            elif used_padding == "ansi":
                ba = ansi_pad(ba, 8)
            else:
                ba = zero_pad(ba, 8)

        self.crypt(ba, used_key, encrypt=True, mode=used_mode, iv=used_iv, pad=False)
        return bytes(ba)
    ...
    def decrypt(
        ...
        *,
        ...
        """
        Расшифровать данные. Возвращает bytes (с учётом снятия паддинга).
        """
        ba = to_bytearray(data)
        used_key = key if key is not None else self._key
        if used_key is None:
            raise ValueError("key is not set (pass key=... to __init__ or decrypt)")
        used_mode = CRYPT_MODE(mode) if mode is not None else self._mode
        used_iv = bytearray(iv) if iv is not None else bytearray(self._iv)
        used_padding = (padding or self._padding).lower()

        self.crypt(ba, used_key, encrypt=False, mode=used_mode, iv=used_iv, pad=False)
    ...
    # ------------------------------------------------------------------
    # Low-level core (сохранён для совместимости)
    # ------------------------------------------------------------------
    ...
    def crypt(
        ...
        """
        Низкоуровневое шифрование/расшифрование in-place.
        ...
        iv_ba = bytearray(iv) if iv is not None else bytearray(self._iv)
        if len(iv_ba) != 8:
...
# ---------------------------------------------------------------------------
# Convenience helpers (оставлены для совместимости)
# ---------------------------------------------------------------------------
...
def encrypt_bytes(
    ...
    return Gost28147(key=key, sbox=sbox, mode=mode, iv=iv, padding=padding).encrypt(message)
...
def decrypt_bytes(
    ...
    return Gost28147(key=key, sbox=sbox, mode=mode, iv=iv, padding=padding).decrypt(ciphertext)
...
def encrypt_hex(
    ...
    return bytes_to_hex(encrypt_bytes(message, key, mode=mode, sbox=sbox, iv=iv, padding=padding))
...
def decrypt_hex(
    ...
    return decrypt_bytes(hex_to_bytes(hex_msg), key, mode=mode, sbox=sbox, iv=iv, padding=padding)
...
if __name__ == "__main__":
    ...
    g = Gost28147(key=key, sbox="cryptopro-a")
